...
exports.install = module.exports.install = function(mongoose, options, Aggregate) {
	...
	var cacheEnabled = options.cache === false ? false : true;
	...
	mongoose.Query.prototype.cache = function(maxAge) {
		this.__cached = cacheEnabled;
		this.__maxAge =  Number(maxAge) ? Number(maxAge) : null;
		log('cache enabled: ', this.__cached, this.__maxAge);
		return this;
	};
	...
	var exec = function(caller, args) {
		...
		function cacheSet(err, obj) {
			if (!err) {
				if(__maxAge) {
					cache.set(key, obj, __maxAge);
				} else {
					cache.set(key, obj);
				}
				log('save to cache: ', key, __maxAge);
			}
